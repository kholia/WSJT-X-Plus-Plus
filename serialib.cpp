...
serialib::serialib()
{
    ...
    // Initialize timeouts structure to zero
...
char serialib::openDevice(const char *Device, const unsigned int Bauds,
    ...
    // Use reasonable defaults that won't cause indefinite blocking
    // ReadIntervalTimeout=MAXDWORD ensures ReadFile returns immediately if no data arrives
    // Individual read functions will override these settings as needed
    timeouts.ReadIntervalTimeout = MAXDWORD;
    timeouts.ReadTotalTimeoutConstant = 0;
    timeouts.ReadTotalTimeoutMultiplier = 0;
    timeouts.WriteTotalTimeoutConstant = 5000;  // 5 second write timeout
...
int serialib::writeChar(const char Byte)
{
    ...
    // Configure write timeout (5 seconds max to prevent indefinite blocking)
    COMMTIMEOUTS writeTimeouts;
    memset(&writeTimeouts, 0, sizeof(writeTimeouts));
    writeTimeouts.WriteTotalTimeoutConstant = 5000;
    writeTimeouts.WriteTotalTimeoutMultiplier = 0;
    if(!SetCommTimeouts(hSerial, &writeTimeouts)) return -1;
...
int serialib::readChar(char *pByte,unsigned int timeOut_ms)
{
    ...
    // Configure timeouts for this read operation
    // Per Microsoft docs:
    // - If all timeouts are zero, ReadFile blocks indefinitely until data arrives
    // - If ReadIntervalTimeout=MAXDWORD and others are 0, ReadFile returns immediately
    COMMTIMEOUTS readTimeouts;
    memset(&readTimeouts, 0, sizeof(readTimeouts));

    if (timeOut_ms == 0) {
        // No timeout requested: return immediately (don't block)
        readTimeouts.ReadIntervalTimeout = MAXDWORD;
        readTimeouts.ReadTotalTimeoutConstant = 0;
        readTimeouts.ReadTotalTimeoutMultiplier = 0;
    } else {
        // Timeout requested: wait up to timeOut_ms for the first byte
        readTimeouts.ReadIntervalTimeout = 0;
        readTimeouts.ReadTotalTimeoutConstant = timeOut_ms;
        readTimeouts.ReadTotalTimeoutMultiplier = 0;
    }

    // Write the parameters, return -1 if an error occured
    if(!SetCommTimeouts(hSerial, &readTimeouts)) return -1;
...
int serialib::readBytes (void *buffer,unsigned int maxNbBytes,unsigned int timeOut_ms, unsigned int sleepDuration_us)
{
    ...
    if (timeOut_ms == 0) {
        ...
        // Timeout requested: wait up to timeOut_ms for data
        readTimeouts.ReadIntervalTimeout = 0;
        readTimeouts.ReadTotalTimeoutConstant = (DWORD)timeOut_ms;
        readTimeouts.ReadTotalTimeoutMultiplier = 0;
    }
