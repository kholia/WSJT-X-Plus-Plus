...
#if defined(Q_OS_WIN32)
  // Wait for device to settle after potential reset
...
void SimpleCATTransceiver::do_modulator_start(QString mode, unsigned, double,
                                              ...
                                              bool, double, double) {
  if (mode.contains("FT4")) {
    pending_digital_mode_ = "FT4";
  } else if (mode.contains("FT2")) {
    pending_digital_mode_ = "FT2";
  } else {
    pending_digital_mode_ = "FT8";
  }
...
QString SimpleCATTransceiver::send_command(QString const &command,
  ...
  port_->flushReceiver();
  QThread::msleep(50);  // Brief pause after flush
  ...
  if (!write_success) {
    throw error{tr("Simple CAT: timeout writing command %1 after %2 attempts")
                .arg(command).arg(max_write_attempts)};
  }

  // Brief pause after write to allow firmware to process
  QThread::msleep(50);
#else
  if (port_->writeString(payload.constData()) != 1) {
