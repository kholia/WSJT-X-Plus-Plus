...
private:
  ...
  bool decode_at_52s_;
  bool kHz_without_k_;
  bool band_hopping_every_minute_;
  ...
  QString hamlib_dll_basename = "libhamlib-4";
...
bool Configuration::logDialFrequency() const {return m_->logDialFrequency_;}
...
bool Configuration::band_hopping_every_minute () const {return m_->band_hopping_every_minute_;}
...
void Configuration::impl::initialize_models ()
{
  ...
  ui_->cbLogDialFrequency->setChecked(logDialFrequency_);
  ...
  ui_->band_hopping_every_minute_check_box->setChecked(band_hopping_every_minute_);
...
void Configuration::impl::read_settings ()
{
  ...
  logDialFrequency_ = settings_->value ("LogDialFrequency", false).toBool ();
  ...
  band_hopping_every_minute_ = settings_->value("BandHoppingEveryMinute",false).toBool ();
...
void Configuration::impl::write_settings ()
{
  ...
  settings_->setValue ("LogDialFrequency", logDialFrequency_);
  ...
  settings_->setValue ("BandHoppingEveryMinute", band_hopping_every_minute_);
...
void Configuration::impl::accept ()
{
  ...
  logDialFrequency_ = ui_->cbLogDialFrequency->isChecked();
  ...
  band_hopping_every_minute_ = ui_->band_hopping_every_minute_check_box->isChecked ();
...
void Configuration::impl::on_hamlib_download_button_clicked (bool /*clicked*/)
{
  ...
  if (hamlib.contains("Hamlib 5")) hamlib_dll_basename = "libhamlib-5";
  QFile f1 {dataPath.absolutePath() + "/" + hamlib_dll_basename + "_old.dll"};
  QFile f2 {dataPath.absolutePath() + "/" + hamlib_dll_basename + "_new.dll"};
  ...
  if (ui_->rbHamlib32->isChecked()) {
    cty_download.configure(network_manager_,
//                           "https://hamlib.sourceforge.net/snapshots-4.7/dll32/libhamlib-4.dll",  // new hamlib 4.7 download location
                           ("http://dr-risse-consulting.de/hamlib/dll32/" + hamlib_dll_basename + ".dll"),  // hamlib mirror DG2YCB
                           dataPath.absoluteFilePath(hamlib_dll_basename + "_new.dll"),
                           "Downloading latest Hamlib DLL file");
  } else {
    cty_download.configure(network_manager_,
//                           "https://hamlib.sourceforge.net/snapshots-4.7/dll64/libhamlib-4.dll",  // new hamlib 4.7 download location
                           ("http://dr-risse-consulting.de/hamlib/dll64/" + hamlib_dll_basename + ".dll"),  // hamlib mirror DG2YCB
                           dataPath.absoluteFilePath(hamlib_dll_basename + "_new.dll"),
                           "Downloading latest Hamlib DLL file");
  }
...
{
  QDir dataPath = QCoreApplication::applicationDirPath();
  MessageBox::warning_message (this, tr ("Error loading Hamlib DLL file"), reason);
...
void Configuration::impl::after_hamlib_downloaded ()
{
  ...
  QString hamlib = QString(QLatin1String(hamlib_version2));
  if (hamlib.contains("Hamlib 5")) hamlib_dll_basename = "libhamlib-5";
  QFile::rename(dataPath.absolutePath() + "/" + hamlib_dll_basename + ".dll", dataPath.absolutePath() + "/" + hamlib_dll_basename + "_old.dll");
  QTimer::singleShot (1000, [=] {
    QFile::rename(dataPath.absolutePath() + "/" + hamlib_dll_basename + "_new.dll", dataPath.absolutePath() + "/" + hamlib_dll_basename + ".dll");
...
void Configuration::impl::on_revert_update_button_clicked (bool /*clicked*/)
{
  ...
  QString hamlib = QString(QLatin1String(hamlib_version2));
  if (hamlib.contains("Hamlib 5")) hamlib_dll_basename = "libhamlib-5";
  QFile f {dataPath.absolutePath() + "/" + hamlib_dll_basename + "_old.dll"};
  if (f.exists()) {
    ui_->revert_update_button->setEnabled (false);
    ui_->hamlib_download_button->setEnabled (false);
    QFile::rename(dataPath.absolutePath() + "/" + hamlib_dll_basename + ".dll", dataPath.absolutePath() + "/" + hamlib_dll_basename + "_new.dll");
    QTimer::singleShot (1000, [=] {
      QFile::copy(dataPath.absolutePath() + "/" + hamlib_dll_basename + "_old.dll", dataPath.absolutePath() + "/" + hamlib_dll_basename + ".dll");
    });
...
void Configuration::impl::display_file_information ()
{
  ...
  ui_->in_use->setText(hamlib);
  if (hamlib.contains("Hamlib 5")) hamlib_dll_basename = "libhamlib-5";
  QFileInfo fi2(dataPath.absolutePath() + "/" + hamlib_dll_basename + "_old.dll");
