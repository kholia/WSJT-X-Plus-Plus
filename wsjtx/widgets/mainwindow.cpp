...
//          ui->tabWidget->setMaximumHeight(225);                       // UR for normal + widescreen
          ...
          ui->ft2Button->setMinimumWidth(50);                         // UR for normal + widescreen
...
//          ui->tabWidget->setMaximumHeight(225);                       // UR for normal + widescreen
          ...
          ui->ft2Button->setMaximumWidth(40);                         // UR for normal + widescreen
          ...
          ui->ft2Button->setMinimumWidth(0);                          // UR for normal + widescreen
...
void MainWindow::setup_status_bar (bool vhf)
{
  ...
  if ("JT9" == m_mode) {
    ...
    mode_label.setStyleSheet ("QLabel{color: #000000; background-color: #ffacda}");
  } else if ("FT4" == m_mode) {
...
void MainWindow::decode()                                       //decode()
{
  ...
  if(m_mode=="FT8") dec_data.params.lft8apon = ui->actionEnable_AP_FT8->isVisible () &&
      ui->actionEnable_AP_FT8->isChecked () && !(m_multithreadFT8 && ui->actionNo_MTD_a8_decodes->isChecked());
...
void MainWindow::decodeDone ()
{
  ...
  if(m_mode.startsWith ("FT") and m_latestDecodeTime>=0 and m_ActiveStationsWidget!=NULL) {
...
void MainWindow::readFromStdout()                             //readFromStdout
{
  ...
  QString all_decodes;
  if(m_ActiveStationsWidget!=NULL) {
    bDisplayPoints=(m_mode.startsWith("FT") and
...
//Left (Band activity) window
      if(!bAvgMsg) {
        if(m_mode=="FT8" and SpecOp::FOX == m_specOp) {
          ...
          if (m_mode.startsWith("FT") and bDisplayPoints and decodedtext1.isStandardMessage()) {
        ...
        if ((m_mode=="FT4" or m_mode=="FT2") && SpecOp::NA_VHF==m_specOp && m_config.NCCC_Sprint() && m_hisCall!="" && m_hisGrid!="" &&
        ...
        if ((m_mode.startsWith ("FT") or m_mode=="Q65" or m_mode=="FST4" or m_mode=="JT65" or m_mode=="JT9" or m_mode=="JT4") && m_hisCall!=""
            && text.contains(" " + m_config.my_callsign() + " " + m_hisCall) && ((!text.contains("73 ")
            && m_config.Wait_features_enabled() && !ui->autoButton->isChecked())
            or ((m_mode=="FT4" or m_mode=="FT2") && SpecOp::NA_VHF==m_specOp && m_config.NCCC_Sprint()))) {
              ...
              auto_tx_mode(true);
              if ((m_mode=="FT4" or m_mode=="FT2") && SpecOp::NA_VHF==m_specOp && m_config.NCCC_Sprint()) {
        ...
        // Wait & Call
        if (wait_and_call && (m_mode.startsWith ("FT") or m_mode=="Q65" or m_mode=="FST4" or m_mode=="JT65" or m_mode=="JT9" or m_mode=="JT4") &&
        ...
        QString text2 = "";
        QStringList tw;
        if (m_mode.startsWith ("FT") or m_mode == "Q65" or m_mode == "FST4") {
        ...
        QTimer::singleShot (100, [=] {
          ...
          if (m_bBestSPArmed && (m_mode=="FT4" or m_mode=="FT2") && CALLING == m_QSOProgress && !ignored && !filtered) {
...
//Right (Rx Frequency) window
      ...
      if(m_mode.startsWith ("FT") or m_mode=="FST4" or m_mode=="Q65") {
        int ftol=10;
...
// multi-station special events.
      ...
      if(m_mode.startsWith ("FT") and SpecOp::FOX!=m_specOp && decodedtext0.string().replace("<","").replace(">","").contains(m_baseCall + " " + m_hisCall)) bDisplayRight=true;  // really all messages for us
...
//### I think this is where we are preventing Hounds from spotting Fox ###
      if(m_mode!="FT8" or (SpecOp::HOUND != m_specOp) or (SpecOp::HOUND == m_specOp and m_config.superFox())) {
        if(m_mode.startsWith ("FT") or m_mode=="Q65"
...
void MainWindow::pskPost (DecodedText const& decodedtext)
{
  ...
  if(m_mode.startsWith ("FT") or m_mode=="MSK144") {
...
void MainWindow::guiUpdate()
{
  ...
  if(m_mode.startsWith ("FT")) icw[0]=0;                        //No CW ID in FT2, FT4 or FT8 mode
...
//----------------------------------------------------------------------
    ...
    if(m_tune or m_mode=="Echo") {
      ...
      if(m_mode=="WSPR") genwspr_(message, msgsent, const_cast<int *> (itone),
                                    (FCL)22, (FCL)22);
      if(m_mode=="MSK144" or m_mode.startsWith ("FT")
    ...
    bool b=(m_mode.startsWith ("FT") or "Q65"==m_mode or "JT65"==m_mode or "JT9"==m_mode) and ui->cbAutoSeq->isVisible ()
...
//----------------------------------------------------------------------
  ...
  if(m_mode.startsWith ("FT") or m_mode=="MSK144" or m_mode=="Q65") {
...
void MainWindow::processMessage (DecodedText const& message, Qt::KeyboardModifiers modifiers)
{
  ...
  if(!m_bFastMode and (!m_config.enable_VHF_features() or m_mode.startsWith ("FT") or m_mode=="FST4")) {
    ...
    if ((Radio::is_callsign (firstcall)
      ...
      if(m_mode != "JT4" && m_mode != "JT65" && !m_mode.startsWith ("JT9") &&
         m_mode != "Q65" && !m_mode.startsWith ("FT") && m_mode!="FST4") {
        return;
      }
    }
  }
...
//      w34=w.at(nw-2);
    ...
    bool bContestOK=(m_mode.startsWith ("FT") or m_mode=="Q65" or m_mode=="MSK144");
    if(message_words.size () > 4   // enough fields for a normal message
      ...
      if(message_words.at(4).contains(grid_regexp) and SpecOp::EU_VHF!=m_specOp) {
        if((SpecOp::NA_VHF==m_specOp or SpecOp::WW_DIGI==m_specOp or
          ...
          if (SpecOp::NA_VHF==m_specOp && (m_mode=="FT4" or m_mode=="FT2") && m_config.NCCC_Sprint()) {
        ...
        if (("RRR" == word_3
          ...
          if(SpecOp::RTTY == m_specOp and m_nextCall!="") {
            ...
            if (false)              // Always Send 73 after receiving RRR or RR73, even in contest mode.
              {
                if (m_config.prompt_to_log() || m_config.autoLog()) {
                  if (!((m_mode=="FT4" or m_mode=="FT2") && SpecOp::NA_VHF==m_specOp && m_config.NCCC_Sprint())) logQSOTimer.start(0);
                }
                else {
                  cease_auto_Tx_after_QSO ();
                }
          ...
          if (m_QSOProgress >= ROGER_REPORT)
            {
              m_QSOProgress = SIGNOFF;
                   ...
                   || (m_QSOProgress >= REPLYING &&
                   (m_mode=="MSK144" or m_mode.startsWith ("FT") || "Q65" == m_mode)))
                  && word_3.startsWith ('R')) {
          m_ntx=4;
          m_QSOProgress = ROGERS;
  ...
  if (!m_bSentReport || base_call != qso_partner_base_call || m_mode == "FT8" || m_mode == "FT4" || m_mode == "FT2"
...
void MainWindow::genCQMsg ()
{
  ...
  if(my_callsign.size () && m_config.my_grid().size ()) {
    ...
    if((m_mode.startsWith ("FT") or m_mode=="MSK144" || "Q65" == m_mode) and
...
{
  return "FT8" == m_mode || "FT4" == m_mode || "FT2" == m_mode || "MSK144" == m_mode
  ...
  if (eme_short_codes) {
    ...
    if (is77BitMode ()) {
      ...
      if(!bHisCall) {
        ...
        if (is77BitMode () && SpecOp::NONE==m_specOp && ((Radio::is_77bit_nonstandard_callsign (my_callsign) && !is_compound) or
            (is_compound && Radio::is_77bit_nonstandard_callsign (hisCall)))) msgtype(t0a, ui->tx1);
      }
      ...
      if (is77BitMode () && SpecOp::NONE==m_specOp && Radio::is_77bit_nonstandard_callsign (my_callsign) && Radio::is_77bit_nonstandard_callsign (hisCall)
          && !(!Radio::is_77bit_nonstandard_callsign (m_baseCall) && hisCall.contains("/P") && !Radio::is_77bit_nonstandard_callsign (hisBase) && my_callsign.contains("/P"))) {
        if (!is_compound && hisCall==hisBase && Radio::is_77bit_nonstandard_callsign (hisCall)) t="<" + hisBase + "> <" + m_baseCall + "> ";
        if (!is_compound && hisCall!=hisBase) t=hisBase + " <" + my_callsign + "> ";
        if ((is_compound or my_callsign.contains("/P") or hisCall!=hisBase) && Radio::is_77bit_nonstandard_callsign (hisBase)) t="<" + hisCall + "> <" + my_callsign + "> ";
        if (is_compound && hisCall.contains("/") && !Radio::is_77bit_nonstandard_callsign (hisBase) && hisCall!=hisBase) t=hisBase + " <" + my_callsign + "> ";
        if (is_compound && !hisCall.contains("/P") && (Radio::is_77bit_nonstandard_callsign (hisBase) or hisCall!=hisBase)) t="<" + hisCall + "> " + m_baseCall + " ";
      }
    ...
    if((m_mode=="MSK144" and !m_bShMsgs) or m_mode.startsWith ("FT") || m_mode == "FST4" || m_mode == "Q65") {
      if(!bHisCall and bMyCall) t=hisCall + " <" + my_callsign + "> " + (m_send_RR73 ? "RR73" : "RRR");
      if(bHisCall and !bMyCall) t="<" + hisCall + "> " + my_callsign + " " + (m_send_RR73 ? "RR73" : "RRR");
    }
    if ((m_mode=="JT4" || m_mode=="Q65") && m_bShMsgs) t="@1500  (RRR)";
    if (is77BitMode () && SpecOp::NONE==m_specOp && Radio::is_77bit_nonstandard_callsign (my_callsign) && !(!Radio::is_77bit_nonstandard_callsign (hisBase) && hisCall.contains("/P"))) {
      if (!is_compound && hisCall==hisBase && Radio::is_77bit_nonstandard_callsign (hisCall)) t=hisBase + " <" + m_baseCall + "> " + (m_send_RR73 ? "RR73" : "RRR");
      if (!is_compound && hisCall!=hisBase) t=hisCall + " <" + my_callsign + "> " + (m_send_RR73 ? "RR73" : "RRR");
      if (is_compound && hisCall!=hisBase) t=hisCall + " <" + my_callsign + "> " + (m_send_RR73 ? "RR73" : "RRR");
      if (is_compound && hisCall==hisBase && Radio::is_77bit_nonstandard_callsign (hisCall)) t=hisBase + " <" + my_callsign + "> " + (m_send_RR73 ? "RR73" : "RRR");
    }
    msgtype(t, ui->tx4);

    t=t0 + "73";
    QString m_lastFullCallsign;
    if((m_mode=="MSK144" and !m_bShMsgs) or m_mode.startsWith ("FT") || m_mode == "FST4" || m_mode == "Q65") {
      if(!bHisCall and bMyCall) t=hisCall + " <" + my_callsign + "> 73";
      if(bHisCall and !bMyCall) t="<" + hisCall + "> " + my_callsign + " 73";
    }
    if (is77BitMode () && SpecOp::NONE==m_specOp && Radio::is_77bit_nonstandard_callsign (my_callsign) && !(!Radio::is_77bit_nonstandard_callsign (hisBase) && hisCall.contains("/P"))) {
      if (!is_compound && hisCall==hisBase && Radio::is_77bit_nonstandard_callsign (hisCall)) t=hisBase + " <" + m_baseCall + "> 73";
      if (!is_compound && hisCall!=hisBase) t=hisCall + " <" + my_callsign + "> 73";
      if (is_compound && hisCall!=hisBase) t=hisCall + " <" + my_callsign + "> 73";
      if (is_compound && hisCall==hisBase && Radio::is_77bit_nonstandard_callsign (hisCall)) t=hisBase + " <" + my_callsign + "> 73";
    }
    if (m_mode=="JT4" || m_mode=="Q65") {
      ...
      if (!keepTx5) msgtype(t, ui->tx5->lineEdit());
    } else if(unconditional || hisBase != m_lastCallsign || !m_lastCallsign.size () || !m_lastFullCallsign.size ()) {
      // only update tx5 when forced or callsign changes
      if (!keepTx5) msgtype(t, ui->tx5->lineEdit());
      m_lastCallsign = hisBase;
      m_lastFullCallsign = hisCall;
    }
  }
...
//### Check this stuff ###
  ...
  if((m_mode=="MSK144" or m_mode.startsWith ("FT") || "Q65" == m_mode) and
...
{
  if((m_mode.startsWith ("FT") or m_mode=="Q65" or m_mode=="FST4" or m_mode=="MSK144") &&
...
void MainWindow::mousePressEvent(QMouseEvent *event)    // mouse press events
{
  ...
  if(ui->pb80->hasFocus() && (event->button() & Qt::RightButton) && (m_mode=="FT8" || m_mode=="FT4" || m_mode=="FT2")) {
    keep_frequency = true;
    setRig(3567000);
    QTimer::singleShot (250, [=] {keep_frequency = false;});
    setXIT (ui->TxFreqSpinBox->value ());
  ...
  if(ui->pb40->hasFocus() && (event->button() & Qt::RightButton) && (m_mode=="FT8" || m_mode=="FT4" || m_mode=="FT2")) {
    keep_frequency = true;
    setRig(7056000);
    QTimer::singleShot (250, [=] {keep_frequency = false;});
    setXIT (ui->TxFreqSpinBox->value ());
  ...
  if(ui->pb30->hasFocus() && (event->button() & Qt::RightButton) && (m_mode=="FT8" || m_mode=="FT4" || m_mode=="FT2")) {
    keep_frequency = true;
    setRig(10131000);
    QTimer::singleShot (250, [=] {keep_frequency = false;});
    setXIT (ui->TxFreqSpinBox->value ());
  ...
  if(ui->pb20->hasFocus() && (event->button() & Qt::RightButton) && (m_mode=="FT8" || m_mode=="FT4" || m_mode=="FT2")) {
    keep_frequency = true;
    setRig(14090000);
    QTimer::singleShot (250, [=] {keep_frequency = false;});
    setXIT (ui->TxFreqSpinBox->value ());
  ...
  if(ui->pb17->hasFocus() && (event->button() & Qt::RightButton) && (m_mode=="FT8" || m_mode=="FT4" || m_mode=="FT2")) {
    keep_frequency = true;
    setRig(18095000);
    QTimer::singleShot (250, [=] {keep_frequency = false;});
    setXIT (ui->TxFreqSpinBox->value ());
  ...
  if(ui->pb15->hasFocus() && (event->button() & Qt::RightButton) && (m_mode=="FT8" || m_mode=="FT4" || m_mode=="FT2")) {
    keep_frequency = true;
    setRig(21091000);
    QTimer::singleShot (250, [=] {keep_frequency = false;});
    setXIT (ui->TxFreqSpinBox->value ());
  ...
  if(ui->pb12->hasFocus() && (event->button() & Qt::RightButton) && (m_mode=="FT8" || m_mode=="FT4" || m_mode=="FT2")) {
    keep_frequency = true;
    setRig(24911000);
    QTimer::singleShot (250, [=] {keep_frequency = false;});
    setXIT (ui->TxFreqSpinBox->value ());
  ...
  if(ui->pb10->hasFocus() && (event->button() & Qt::RightButton) && (m_mode=="FT8" || m_mode=="FT4" || m_mode=="FT2")) {
    keep_frequency = true;
    setRig(28091000);
    QTimer::singleShot (250, [=] {keep_frequency = false;});
    setXIT (ui->TxFreqSpinBox->value ());
...
void MainWindow::displayWidgets(qint64 n)
{
  ...
  ui->pbBestSP->setVisible(m_mode=="FT4" or m_mode=="FT2");
  b=false;
  if(m_mode.startsWith ("FT") || "Q65" == m_mode) {
...
void MainWindow::on_actionFT4_triggered()
{
  ...
  QTimer::singleShot (50, [=] {
    ...
    on_sbSubmode_valueChanged(ui->sbSubmode->value());
    ui->cbHoldTxFreq->setChecked (HoldTxFreqStatus);
    if (ui->cbCQonly->isChecked()) {  // This is necessary to get FT4 decodes when the program is started with CQonly enabled. Why?
...
{
  //Prevent program crashes when switching to FT2
  monitor(false);
  to_jt9(m_ihsym,-1,-1);
  decodeDone();
  ...
  QTimer::singleShot (50, [=] {
    ...
    on_sbSubmode_valueChanged(ui->sbSubmode->value());
    ui->cbHoldTxFreq->setChecked (HoldTxFreqStatus);
    if (ui->cbCQonly->isChecked()) {  // This is necessary to get FT2 decodes when the program is started with CQonly enabled. Why?
      ui->cbCQonly->setChecked(false);
      ui->cbCQonly->setChecked(true);
    }
  });
  ...
  m_nsps=3456;                    //For widescraph speed
  m_FFTSize = m_nsps/2;
  if (m_tci_audio) Q_EMIT m_config.transceiver_blocksize (m_FFTSize);
  else Q_EMIT FFTSize (m_FFTSize);
  m_hsymStop=21;
...
{
  if (m_transmitting && (m_mode=="FT4" or m_mode=="FT2")) return;  // we can't allow changing Tx frequency while transmitting for FT4 and FT2
...
//  if (ui->cbHoldTxFreq->isChecked ()) ui->RxFreqSpinBox->setValue(n);
  ...
  // UK stations must transmit below 5358 kHz or above 5362 kHz on 60m
  if((m_config.my_callsign().startsWith("G") or m_config.my_callsign().startsWith("M")
      or m_config.my_callsign().startsWith("2")) && (m_freqNominal + ui->TxFreqSpinBox->value()) > 5357850
      && (m_freqNominal + ui->TxFreqSpinBox->value()) < 5362000) {
    QString band=m_config.bands()->find(m_freqNominal);
    if(band=="60m") {
      if (m_freqNominal + ui->TxFreqSpinBox->value() < 5361000) {
        ui->TxFreqSpinBox->setValue(5357850 - m_freqNominal);
        m_wideGraph->setTxFreq(5357850 - m_freqNominal);
      }
      if (m_freqNominal + ui->TxFreqSpinBox->value() > 5361000) {
        ui->TxFreqSpinBox->setValue(5362000 - m_freqNominal);
        m_wideGraph->setTxFreq(5362000 - m_freqNominal);
      }
      QTimer::singleShot (0, [=] {   // don't block guiUpdate
        MessageBox::information_message (this, tr ("UK stations must transmit below 5358 kHz or above 5362 kHz on 60m."));
      });
    }
  }
  statusUpdate ();
...
void MainWindow::setXIT(int n, Frequency base)
{
  ...
  if (!(m_bSimplex || (SpecOp::FOX==m_specOp && m_config.superFox()))) {
    ...
    if (m_config.split_mode () && (!m_config.enable_VHF_features () ||
        m_mode.startsWith ("FT") || m_mode=="FST4")) {
...
void MainWindow::setFreq4(int rxFreq, int txFreq)
{
  ...
  if(m_mode=="WSPR" or m_mode=="FST4W") {
    ...
    if (ui->TxFreqSpinBox->isEnabled ()) {
      ...
      if (m_mode.startsWith ("FT") || m_mode=="FST4")
        {
...
void MainWindow::transmit (double snr)
{
  ...
  if(m_mode.startsWith ("FT") and m_maxPoints>0 and SpecOp::ARRL_DIGI==m_specOp) {
    ui->dxCallEntry->setText(m_deCall);
    ui->dxGridEntry->setText(m_deGrid);
    genStdMsgs("-10");
  }
...
{
  ui->respondComboBox->setVisible((m_mode.startsWith ("FT") or m_mode=="FST4"
...
{
  if (m_ActiveStationsWidget != NULL and (m_mode == "Q65" or m_mode.startsWith ("FT"))) {
    if (m_specOp == SpecOp::Q65_PILEUP) {
...
void MainWindow::bandHoppingTimer()
{
    ...
    switch (startIndex){
    case 0:
      if (m_config.band_hopping_every_minute()) {
          m_displayBand = false;
          bandHopping();
          startIndex = 0;
          return;
...
void MainWindow::check_button_color()
{
  ...
  if((((m_mode.startsWith ("FT") or m_mode=="Q65" or m_mode=="FST4" or m_mode=="MSK144" or m_mode=="JT65" or m_mode=="JT9" or m_mode=="JT4") &&
  ...
  if (m_mode=="FT2") {
    ui->ft2Button->setStyleSheet("QPushButton {background-color: #00ff00; color: #000000; border: 1px solid #32414B; border-radius: 5px; padding: 3px; outline: none; min-width: 3em;}");
  } else {
    if (m_useDarkStyle) {
      ui->ft2Button->setStyleSheet("QPushButton {background-color: #505F69; border: 1px solid #32414B; color: #F0F0F0; border-radius: 4px; padding: 3px; outline: none; min-width: 3em;}");
    } else {
      ui->ft2Button->setStyleSheet("QPushButton {background-color: #e1e1e1; border: 1px solid #adadad; border-radius: 0px; padding: 3px; outline: none; min-width: 3em;}");
    }
  }
  if (m_mode=="MSK144") {
