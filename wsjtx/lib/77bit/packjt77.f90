...
! These variables are accessible from outside via "use packjt77":
  ...
  integer :: nzhash=0
  integer n28a,n28b
  ! variables from packjt77var
  character (len=13), dimension(0:1023) ::  calls10var=''
  character (len=13), dimension(0:4095) ::  calls12var=''
  character (len=13), dimension(1:MAXHASHVAR) :: calls22var=''
  character (len=13), dimension(0:1023) ::  txcalls10var=''
  character (len=13), dimension(0:4095) ::  txcalls12var=''
  character (len=13), dimension(1:MAXTXHASHVAR) :: txcalls22var=''
  character (len=13), dimension(1:MAXRECENTVAR) :: recent_callsvar=''
  character (len=13), dimension(1:840) :: last_callsvar=''
  character (len=13) :: mycall13var=''
  character (len=13) :: dxcall13var=''
  character (len=13) :: mycall13_0var=''
  character (len=13) :: dxcall13_0var=''
  integer hashmy10var,hashmy12var,hashmy22var,hashdx10var
  logical :: dxcall13_setvar=.false.
  logical :: mycall13_setvar=.false.
  integer, dimension(1:MAXHASHVAR) :: ihash22var=-1
  integer, dimension(1:5) :: itxhash22var=-1
  integer, dimension(1:24) :: nlast_callsvar=0
  integer, dimension(1:25) :: nthrindexvar=(/0,200,300,370,420,460,500,530,560,590,610,630,650,670,690,710, &
                                        730,750,770,790,800,810,820,830,840/)
  integer :: nzhashvar=0
  integer :: nztxhashvar=0
  integer n28avar,n28bvar
  logical(1) lcommonft8b
!$omp threadprivate(recent_callsvar,n28avar,n28bvar)
...
subroutine hash10var(n10,c13,nthr)
  ...
  if(nthr.gt.25) then ! TX message
    if(len(trim(txcalls10var(n10))).gt.0) then
      c13=txcalls10var(n10)
      c13='<'//trim(c13)//'>'
    endif
  else
    if(len(trim(calls10var(n10))).gt.0) then
      c13=calls10var(n10)
      c13='<'//trim(c13)//'>'
    endif
  endif
...
end subroutine hash10var

subroutine hash12var(n12,c13,nthr)
  ...
  if(n12.lt.0 .or. n12.gt.4095) return
  if(nthr.gt.25) then ! TX message
    if(len(trim(txcalls12var(n12))).gt.0) then
      c13=txcalls12var(n12)
      c13='<'//trim(c13)//'>'
    endif
  else
    if(len(trim(calls12var(n12))).gt.0) then
      c13=calls12var(n12)
      c13='<'//trim(c13)//'>'
    endif
  endif
...
end subroutine hash12var

subroutine hash22var(n22,c13,nthr)
...
1900 continue
  ...
  return 
end subroutine save_hash_mycallvar

subroutine save_hash_txcallvar(c13,n10,n12,n22,lhashit)
...
! New entry: move table down, making room for new one at the top
    itxhash22var(nztxhashvar:2:-1)=itxhash22var(nztxhashvar-1:1:-1)

! Add the new entry
    txcalls22var(nztxhashvar:2:-1)=txcalls22var(nztxhashvar-1:1:-1)
    itxhash22var(1)=n22
    txcalls22var(1)=cw
    if(nztxhashvar.lt.MAXTXHASHvar) nztxhashvar=nztxhashvar+1

1900 continue
  endif

  return 
end subroutine save_hash_txcallvar

subroutine save_hash_callvar(c13,nthr)
  ...
  nposition=nthrindexvar(nthr)+nlast_callsvar(nthr)
  if(nposition.lt.nthrindexvar(nthr+1)) then
    nlast_callsvar(nthr)=nlast_callsvar(nthr)+1
    last_callsvar(nposition+1)=cw
  endif

  return 
end subroutine save_hash_callvar

subroutine pack77var(msg0,i3,n3,c77,ntxhash)
...
! Check 0.1 (DXpedition mode)
  call pack77_01var(nwords,w,i3,n3,c77,ntxhash)
  if(i3.ge.0 .or. n3.ge.1) go to 900
! Check 0.2 (EU VHF contest exchange)
!  call pack77_02(nwords,w,i3,n3,c77)
!  if(i3.ge.0) go to 900
...
! Check 0.3 and 0.4 (ARRL Field Day exchange)
  call pack77_03var(nwords,w,i3,n3,c77,ntxhash)
  if(i3.ge.0) go to 900
  if(nwords.ge.2) go to 100
...
100 call pack77_06var(nwords,w,i3,n3,c77,i3_hint,n3_hint,ntxhash)
  if(i3.ge.0) go to 900
...
! Check Type 3 (ARRL RTTY contest exchange)
  call pack77_3var(nwords,w,i3,n3,c77,ntxhash)
  if(i3.ge.0) go to 900

! Check Type 4 (One nonstandard call and one hashed call)
  call pack77_4var(nwords,w,i3,n3,c77,ntxhash)
  if(i3.ge.0) go to 900

! Check Type 5 (EU VHF Contest with 2 hashed calls, report, serial, and grid6)
  call pack77_5var(nwords,w,i3,n3,c77,ntxhash)
  if(i3.ge.0) go to 900
...
900 return
end subroutine pack77var

subroutine unpack77var(c77,nrx,msg,unpk77_successvar,nthr)
!
! nrx=1 when unpacking a received message
! nrx=0 when unpacking a to-be-transmitted message
...
! 0.1  K1ABC RR73; W9XYZ <KH1/KH7Z> -11   28 28 10 5       71   DXpedition Mode
     read(c77,1010) n28avar,n28bvar,n10,n5
1010 format(2b28,b10,b5)
     irpt=2*n5 - 30
     write(crpt,1012) irpt
1012 format(i3.2)
     ...
     call hash10var(n10,call_3,nthr)
     if(call_3.eq.'<...>') then
       call hash10(n10,call_3)
     endif
     
     if(nrx.eq.1) then
       if(dxcall13_setvar .and. len(trim(dxcall13var)).ge.3 .and. hashdx10var.eq.n10) then; call_3='<'//trim(dxcall13var)//'>'
         elseif(.not.dxcall13_setvar .and. len(trim(dxcall13var)).ge.3  .and. .not.lcommonft8b .and. hashdx10var.eq.n10) then
           call_3='<'//trim(dxcall13_0var)//'>'
         elseif(.not.dxcall13_setvar .and. len(trim(dxcall13var)).ge.3  .and. .not.lcommonft8b .and. hashdx10var.ne.n10) then
           call_3=trim(call_3)
         elseif(.not.dxcall13_setvar .and. len(trim(dxcall13var)).ge.3  .and. lcommonft8b .and. hashdx10var.eq.n10)  then
           call_3='<'//trim(dxcall13_0var)//'>'
         elseif(.not.dxcall13_setvar .and. len(trim(dxcall13var)).ge.3  .and. lcommonft8b .and. hashdx10var.ne.n10)  then
           call_3='<...>'
       endif
     endif
     if(nrx.eq.0 .and. mycall13_setvar .and. n10.eq.hashmy10var) call_3='<'//trim(mycall13var)//'>'
     msg=trim(call_1)//' RR73; '//trim(call_2)//' '//trim(call_3)//' '//crpt
...
! 0.3   WA9XYZ KA1ABC R 16A EMA            28 28 1 4 3 7    71   ARRL Field Day
! 0.4   WA9XYZ KA1ABC R 32A EMA            28 28 1 4 3 7    71   ARRL Field Day
     read(c77,1030) n28avar,n28bvar,ir,intx,nclass,isec
1030 format(2b28,b1,b4,b3,b7)
     if(isec.gt.NSEC .or. isec.lt.1) then
         unpk77_successvar=.false.
         isec=1
...
2010    format(b28.28,b15.15,b5.5)
        ...
        write(crpt,'(i3)') idbm
        msg=trim(call_1)//' '//grid4//' '//trim(adjustl(crpt))
        if (unpk77_successvar) call save_hash_callvar(call_1,nthr) !### Is this OK here? ###
...
! Prefix
           ...
           msg=trim(adjustl(cpfx))//'/'//trim(call_1)//' '//trim(adjustl(crpt))
           call_1a=trim(adjustl(cpfx))//'/'//trim(call_1)
           call save_hash_callvar(call_1a,nthr)  !### Is this OK here? ###
        else
! Suffix
           npfx=npfx-nzzz
           if(npfx.le.35) then
...
1000 format(2(b28,b1),b1,b15,b3)
     call unpack28var(n28avar,call_1,unpk28_success,nthr)
     if(nrx.eq.1 .and. mycall13_setvar .and. hashmy22var.eq.(n28avar-2063592)) then
        call_1='<'//trim(mycall13var)//'>'
        unpk28_success=.true.
     endif
     if(.not.unpk28_success) unpk77_successvar=.false.
     call unpack28var(n28bvar,call_2,unpk28_success,nthr)
     ...
     if(index(call_1,'<').le.0) then
        ...
        if(i.ge.4 .and. ipa.eq.1 .and. i3.eq.1) call_1(i:i+1)='/R'
        if(i.ge.4 .and. ipa.eq.1 .and. i3.eq.2) call_1(i:i+1)='/P'
        if(i.ge.4) call add_call_to_recent_callsvar(call_1,nthr)
     endif
     if(index(call_2,'<').le.0) then
        i=index(call_2,' ')
        if(i.ge.4 .and. ipb.eq.1 .and. i3.eq.1) call_2(i:i+1)='/R'
        if(i.ge.4 .and. ipb.eq.1 .and. i3.eq.2) call_2(i:i+1)='/P'
        if(i.ge.4) then
        call add_call_to_recent_callsvar(call_2,nthr)
! only hash the "from" call
           call save_hash_callvar(call_2,nthr)
        endif
     endif
     if(igrid4.le.MAXGRID4) then
        call to_grid4var(igrid4,grid4,unpkg4_success)
...
!000102 -23  0.5  801 ~ <...> NV3QHV/R JF62
        if(ir.eq.0 .and. call_1(1:1).eq.'<' .and. grid4.ne.'RR73') then ! hash value can also be associated with a valid callsign
          ...
          if(nlencall2.gt.1) then
            if(call_2(nlencall2-1:nlencall2).eq.'/R') unpk77_successvar=.false.
          endif
        endif
     else
        irpt=igrid4-MAXGRID4
...
! Type 3: ARRL RTTY Contest
     read(c77,1040) itu,n28avar,n28bvar,ir,irpt,nexch,i3
1040 format(b1,2b28.28,b1,b3.3,b13.13,b3.3)
     write(crpt,1042) irpt+2
1042 format('5',i1,'9')
     nserial=nexch
...
1050 format(b12,b58,b1,b2,b1)
     do i=11,1,-1
        ...
        n58=n58/38
     enddo
     call hash12var(n12,call_3,nthr)
     if(call_3.eq.'<...>') then
       call hash12(n12,call_3)
     endif
     if(iflip.eq.0) then       ! 12 bit hash for TO call
        ...
        call add_call_to_recent_callsvar(call_2,nthr)
        if(nrx.eq.1 .and.                        &  
           dxcall13_setvar .and. mycall13_setvar .and. & 
           call_2.eq.dxcall13var .and.              &
           n12.eq.hashmy12var ) call_1='<'//trim(mycall13var)//'>'
        if(nrx.eq.1 .and.                        &
           mycall13_setvar .and.                    & 
           index(call_1,'<...>').gt.0 .and.      &
           n12.eq.hashmy12var ) call_1='<'//trim(mycall13var)//'>'
        ...
        call_1=adjustl(c11)
        call_2=call_3          
        call add_call_to_recent_callsvar(call_1,nthr)
        if(nrx.eq.0 .and.                        & 
           mycall13_setvar .and. & 
           n12.eq.hashmy12var) call_2='<'//trim(mycall13var)//'>'
     endif
     if(icq.eq.0) then
        if(nrpt.eq.0) msg=trim(call_1)//' '//trim(call_2)
        if(nrpt.eq.1) msg=trim(call_1)//' '//trim(call_2)//' RRR'
     ...
     if(call_2(1:1).lt.':' .and. call_2(1:1).gt.'/' .and. call_2(2:2).lt.':' .and. call_2(2:2).gt.'/') unpk77_successvar=.false.
     if(len_trim(call_2).eq.11) then
       ispace=index(call_2,' '); if(ispace.gt.0 .and. ispace.lt.12) unpk77_successvar=.false.
       islash=index(call_2,'/'); if(islash.eq.1 .or. islash.eq.2 .or. islash.eq.11 .or. (islash.eq.10 .and. &
       call_2(11:11).gt.'@' .and. call_2(11:11).lt.'[' .and. call_2(11:11).ne.'P')) unpk77_successvar=.false.
       if(islash.lt.6 .and. call_2(11:11).lt.':' .and. call_2(11:11).gt.'/') unpk77_successvar=.false.
     endif
     if(msg(1:3).ne.'CQ ') then
       if(call_1(1:1).lt.':' .and. call_1(1:1).gt.'/' .and. call_1(2:2).lt.':' .and.call_1(2:2).gt.'/') unpk77_successvar=.false.
       if(len_trim(call_1).eq.11) then
         ispace=index(call_1,' '); if(ispace.gt.0 .and. ispace.lt.12) unpk77_successvar=.false.
         islash=index(call_1,'/'); if(islash.eq.1 .or. islash.eq.2 .or. islash.eq.11 .or. (islash.eq.10 .and. &
         call_1(11:11).gt.'@' .and. call_1(11:11).lt.'[' .and. call_1(11:11).ne.'P')) unpk77_successvar=.false.
         if(islash.lt.6 .and. call_1(11:11).lt.':' .and. call_1(11:11).gt.'/') unpk77_successvar=.false.
       endif
     endif
     nmsglen=len_trim(msg)
! protocol violations, also reported at wrong TX message packing
! prevent broken message transmission
     if(unpk77_successvar .and. nmsglen.gt.0) then
       indxp=index(msg,'/P ')
...
! with exception to broken by packing message 'JQ1AHT/P <JL1LOF/1> -15' wrongly being transmitted as type 4 message 'JQ1AHT/P <JL1LOF/1>'
         if(msg(nmsglen:nmsglen).eq.'>') unpk77_successvar=.false.
       endif
     endif
! <...> /BZZ/0ZZ/C
! <...> ZZZZ/0Z/Z/
...
! <...> 0I/IZZZ/O K
! <...> J/IZZZ/O K
     if(unpk77_successvar .and. iflip.eq.0 .and. icq.eq.0 .and. nrpt.eq.0) then
       nlencall2=len_trim(call_2)
       if(nlencall2.gt.9) then
         if(call_2(1:1).eq.'/' .or. call_2(nlencall2:nlencall2).eq.'/') unpk77_successvar=.false.
         nindxspace=index(call_2,' ')
         if(nindxspace.gt.0 .and. nindxspace.lt.nlencall2) unpk77_successvar=.false.
       endif
     endif
...
1060 format(b12,b22,b1,b3,b11,b25)
     ...
     call hash12var(n12,call_1,nthr)
     if(call_1.eq.'<...>') then
       call hash12(n12,call_1)
     endif
     if(n12.eq.hashmy12var) call_1='<'//trim(mycall13var)//'>'
     call hash22var(n22,call_2,nthr)
     if(call_2.eq.'<...>') then
       call hash22(n22,call_2)
     endif
     nrs=52+irpt
...
1022 format(i2,i4.4)
  ...
  return
end subroutine unpack77var

subroutine pack28var(c13,n28,ntxhash)
...
! Check for <...> callsign
  if(c13(1:1).eq.'<')then
     call save_hash_txcallvar(c13,n10,n12,n22,lhashit)   !Save callsign in hash table
     i2=index(c13,'>')
     c13=c13(2:i2-1)
     n22=ihashcall(c13,22)
     n28=NTOKENS + n22
...
! This is a 22-bit hash of a callsign
     n22=n28
     call hash22var(n22,c13,nthr)     !Retrieve callsign from hash table
     if(c13.eq.'<...>') then
       call hash22(n22,c13)
     endif
     go to 900
...
subroutine pack77_01var(nwords,w,i3,n3,c77,ntxhash)
...
! Type 0.1:  K1ABC RR73; W9XYZ <KH1/KH7Z> -11   28 28 10 5       71   DXpedition special msg
  i3=0
  n3=1
  call pack28var(w(1),n28avar,ntxhash)
  call pack28var(w(3),n28bvar,ntxhash)
  lhashit=.false.; if(ntxhash.eq.1) lhashit=.true.
  call save_hash_txcallvar(w(4),n10,n12,n22,lhashit)
  i2=index(w(4),'>')
  c13=w(4)(2:i2-1)
  n10=ihashcall(c13,10)
  write(c77,1010) n28avar,n28bvar,n10,n5,n3,i3
1010 format(2b28.28,b10.10,b5.5,2b3.3)
  
900 return
end subroutine pack77_01var

subroutine pack77_03var(nwords,w,i3,n3,c77,ntxhash)
...
! 0.4   WA9XYZ KA1ABC R 32A EMA            28 28 1 4 3 7    71   ARRL Field Day
  ...
  call pack28var(w(1),n28avar,ntxhash)
  call pack28var(w(2),n28bvar,ntxhash)
  ir=0
  if(w(3)(1:2).eq.'R ') ir=1
  write(c77,1010) n28avar,n28bvar,ir,intx,nclass,isec,n3,i3
1010 format(2b28.28,b1,b4.4,b3.3,b7.7,2b3.3)

  return
end subroutine pack77_03var

subroutine pack77_06var(nwords,w,i3,n3,c77,i3_hint,n3_hint,ntxhash)
...
! WSPR Type 1
     ...
     i3=0
     n3=6
     call pack28var(w(1),n28,ntxhash)
     grid4=w(2)(1:4)
     k1=(ichar(grid4(1:1))-ichar('A'))*18*10*10
     k2=(ichar(grid4(2:2))-ichar('A'))*10*10
     k3=(ichar(grid4(3:3))-ichar('0'))*10
...
! Third character of a suffix must be a digit
     ...
     i3=0
     n3=6
     call pack28var(bcall//'       ',n28,ntxhash)
     read(w(2),*) idbm
     if(idbm.lt.0) idbm=0
     if(idbm.gt.60) idbm=60
     idbm=nint(0.3*idbm)
...
900 return  
end subroutine pack77_06var

subroutine pack77_1var(nwords,w,i3,n3,c77,ntxhash)
...
10 i1psuffix=index(w(1)//' ' ,'/P ')
  ...
  c13=bcall_1
  if(c13(1:3).eq.'CQ_' .or. w(1)(1:1).eq.'<') c13=w(1)
  call pack28var(c13,n28avar,ntxhash)
  c13=bcall_2
  if(w(2)(1:1).eq.'<') c13=w(2)
  call pack28var(c13,n28bvar,ntxhash)
  ipa=0
  ipb=0
  if(i1psuffix.ge.4.or.index(w(1)//' ','/R ').ge.4) ipa=1
  if(i2psuffix.ge.4.or.index(w(2)//' ','/R ').ge.4) ipb=1
...
1    mult='   '
     ...
     if(mult.ne.'   ' .or. nserial.gt.0) then
        ...
        itu=0
        if(trim(w(1)).eq.'TU;') itu=1
        call pack28var(w(1+itu),n28avar,ntxhash)
        call pack28var(w(2+itu),n28bvar,ntxhash)
        ir=0
        if(w(3+itu)(1:2).eq.'R ') ir=1
        read(w(3+itu+ir),*,err=900) irpt
        irpt=(irpt-509)/10 - 2
...
! 3     TU; W9XYZ K1ABC R 579 MA             1 28 28 1 3 13       74   ARRL RTTY contest
! 3     TU; W9XYZ G8ABC R 559 0013           1 28 28 1 3 13       74   ARRL RTTY (DX)
        write(c77,1010) itu,n28avar,n28bvar,ir,irpt,nexch,i3
1010    format(b1,2b28.28,b1,b3.3,b13.13,b3.3)
     endif
  endif

900 return
end subroutine pack77_3var

subroutine pack77_4var(nwords,w,i3,n3,c77,ntxhash)
...
! Example message: <WA9XYZ> PJ4/KA1ABC RR73           12 58 1 2 1      74
  ...
  if(nwords.eq.2 .or. nwords.eq.3) then
     ...
     if(icq.eq.1) then
        ...
        n12=0
        c11=adjustr(call_2(1:11))
        call save_hash_txcallvar(w(2),n10,n12,n22,lhashit)
     else if(w(1)(1:1).eq.'<') then
        iflip=0
        i3=4
        call save_hash_txcallvar(w(1),n10,n12,n22,lhashit)
        c11=adjustr(call_2(1:11))
     else if(w(2)(1:1).eq.'<') then
        iflip=1
        i3=4
...
900 return
end subroutine pack77_4var

subroutine pack77_5var(nwords,w,i3,n3,c77,ntxhash)
...
! Type 0.2: <PA3XYZ> <G4ABC/P> R 590003 IO91NP     h10 h20 r1 s3 s12 g25
  ...
  call save_hash_txcallvar(w(2),n10a,n12a,n22,lhashit)
  i2=index(w(2),'>')
  c13=w(2)(2:i2-1)
  n22=ihashcall(c13,22)
...
! only add if the callsign is not already on the list
  ladd=.true.
  do i=1,MAXRECENT-1 ! if callsign is at the end of the list add it again
     if(recent_callsvar(i).eq.callsign) ladd=.false.
  enddo

  if(ladd) then
     do i=MAXRECENT,2,-1
        recent_callsvar(i)=recent_callsvar(i-1)
     enddo
     recent_callsvar(1)=callsign
  endif

! Make sure that callsign is hashed
  call save_hash_callvar(callsign,nthr)
